...
from accounts.forms import  UserLogin,UserAddForm
LEAVES_TOTAL=30
...
def leave_creation(request):
	...
	if request.method == 'POST':
		...
		if form.is_valid():
			...
			user = request.user
			instance.user = user
			jet=instance.reason = request.POST.get('reason')

			# assigned_user = User.objects.get(id = email)
			# print(assigned_user)
			...
			instance.save()
			print(jet)
			x='ishaan.mandliya@somaiya.edu'
			send_mail(
			 	user,
             	jet,
             	'ishaan.mandliya@somaiya.edu',
             	['veersheth450@gmail.com'],
           	 	 fail_silently=False,
			 )
